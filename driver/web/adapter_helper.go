...
type apiDataType interface {
	model.Nudge | model.AssignmentGroup | model.NudgesProcess | model.UnitWithTimezone | model.Unit | model.User | model.Module | model.ProviderCourse | model.NudgesConfig | string | model.UserCourse | model.Content | model.Course | model.SentNudge
}

// requestDataType represents any data type that may be sent in an API request body
type requestDataType interface {
	Def.AdminReqUpdateUnit | Def.AdminReqCreateNudge | Def.NudgesConfig | Def.AdminReqUpdateModule | Def.AdminReqUpdateCourse | Def.AdminReqUpdateNudge | apiDataType
}
