...
type apiDataType interface {
	model.UnitWithTimezone | model.User | model.NudgesConfig | model.Nudge | model.SentNudge | model.AssignmentGroup | model.Unit | model.UserCourse | model.NudgesProcess | string | model.Module | model.ProviderCourse | model.Course | model.Content
}

// requestDataType represents any data type that may be sent in an API request body
type requestDataType interface {
	Def.AdminReqUpdateNudge | Def.AdminReqCreateNudge | Def.NudgesConfig | apiDataType
}
